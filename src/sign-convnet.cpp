...
void BMPFilesToSamples(vector<string> files, int features, int classes,
                       MatrixXf* samplesX, MatrixXf* samplesY) {
    ...
    *samplesY = MatrixXf(classes, files.size());
    ...
    for (vector<string>::iterator it = files.begin(); it != files.end(); ++it) {
        ...
        MatrixXf datacol =
            Map<Matrix8u>(data.data(), data.size(), 1).cast<MatrixXf::Scalar>();

        datacol = (datacol - MatrixXf::Ones(data.size(), 1) * 128) /
        ...
        i++;
        if (i % 1000 == 0) cout << "Read " << i << " files." << endl;
...
int main() {
    ...
    if (Eigen::nbThreads() == 1)
        cout << "Warning! using only one thread! make sure to use -fopenmp "
                "with G++"
             << endl;
