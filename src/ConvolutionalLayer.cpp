...
SpMat ConvolutionalLayer::buildIM2COL() {
	...
	SpMat x(outWidth * outHeight * K * K * dimension, width * height * dimension);
			...
			for(int yy = 0; yy < K; ++yy) {
				for(int xx = 0; xx < K; ++xx) {
					for (int dd = 0; dd<dimension; ++dd) {
						...
						tripletList.push_back(Triplet(outindex_base + (yy*K + xx)*dimension + dd, inindex, 1));
					}
				}
			}
		}
...
Eigen::MatrixXf ConvolutionalLayer::forward(const Eigen::MatrixXf& input) {
	...
	//reshape the im2col to K*K*D by W2*H2 matrix to prepare it for the convolution
	...
	//then add the bias.
	//the im2col is K*K*D by W2*H2 and our convolution filter bank is N by K*K*D.
	Eigen::MatrixXf outputImages = w * inp + b * Eigen::VectorXf::Ones(inp.cols()).transpose();
	...
	//reshape our output images to the usual one-column-per-image format, just like the input
	s = Eigen::MatrixXf(Eigen::Map<Eigen::MatrixXf>(outputImages.data(), outputImages.cols() * outputImages.rows() / input.cols(), input.cols()));
...
Eigen::MatrixXf ConvolutionalLayer::backprop(const Eigen::MatrixXf& error) {
	...
	Eigen::MatrixXf backprop_im2col_shape(Eigen::Map<Eigen::MatrixXf>(backprop_im2col.data(), backprop_im2col.cols() * backprop_im2col.rows() / error.cols(), error.cols()));
